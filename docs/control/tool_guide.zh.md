...
<div style="display:flex; gap:24px; align-items:flex-start; flex-wrap:wrap;">
  <div style="flex:0 0 38%; min-width:180px;">
    <img src="/assets/control/toolguide/intro.png" alt="界面介绍" style="width:100%; height:auto; display:block;" />
  </div>
...
- Windows：右键开始图标 → 设备管理器 → 展开"端口（COM 与 LPT）"
- 查找"USB 串行设备"对应端口（例如 `COM20`、`COM31`）

![COM示意图](../assets/control/toolguide/com.png)

打开 `bxi_tool` 后，在左上角端口下拉框选择对应端口。若无选项，点击右侧刷新按钮。

![连接刷新示意图](../assets/control/toolguide/connectrefreash.png)

### 2）选择目标

![接线示意图](../assets/control/toolguide/connect.png)
...
![实例示意图](../assets/control/toolguide/9.png)
...
![电机上电输出示例](../assets/control/toolguide/boot.png)
...
![串口界面截图](../assets/control/toolguide/serial.png)
...
![调试界面截图](../assets/control/toolguide/debug.png)
...
![配置界面截图](../assets/control/toolguide/config.png)

连接上电机后，进入左侧菜单 `Config` 页面可修改参数。

### 5.1 CAN BUS 配置

下翻至 **CAN BUS** 区域可修改 CAN 通信参数。
...
![CAN BUS 参数区截图](../assets/control/toolguide/canbus.png)

### 5.2 参数保存

若希望参数掉电不丢失，请在修改后使用右下角"保存参数"功能。

![保存参数按钮截图](../assets/control/toolguide/configsave.png)
...
![校准界面截图](../assets/control/toolguide/calibration.png)
...
![固件烧录界面截图](../assets/control/toolguide/firmware.png)
